...
export async function POST(req:NextRequest){
 try{
  ...
  if(!/^[a-zA-Z0-9-]{2,80}$/.test(conversationId))return response({error:'Conversación no válida.'},400);await submitChat(person,{id,text,mode,files,conversationId});return response({person,messages:await messages(person,conversationId)});
