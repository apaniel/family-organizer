...
import {readRecords,saveRecord,saveCompletedTask,removeRecord} from '@/lib/family-mission/store';
...
export async function POST(req:NextRequest){if(!(await requireCalendarSyncRouteAuth(req)).authorized)return response({error:'Acceso no autorizado.'},401);
 try{const raw=await req.json();if(JSON.stringify(raw).length>30000)return response({error:'El registro es demasiado grande.'},400);return response(raw.completeOn!==undefined?await saveCompletedTask(raw):{record:await saveRecord(raw)});}catch(e){return response({error:e instanceof Error && !/SQL|D1|constraint/i.test(e.message)?e.message:'No se pudo guardar. Revisa los datos.'},400);}}
