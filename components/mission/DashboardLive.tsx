...
import {chatIsPending,refreshOnReturn} from './dashboard-refresh';
export default function DashboardLive({editing}:{editing:boolean}){
 ...
 const focus=()=>{if(document.visibilityState==='visible'){refreshOnReturn();void check();}};
 void check();const timer=setInterval(()=>void check(),3000);document.addEventListener('visibilitychange',focus);window.addEventListener('focus',focus);
