...
export type AttentionReason='overdue'|'waiting'|'unconfirmed'|'unassigned';
export const attentionLabels:Record<AttentionReason,string>={overdue:'Vencidas',waiting:'Esperando respuesta',unconfirmed:'Sin confirmar',unassigned:'Sin responsable'};
...
export function attentionReport(records:FamilyRecord[],today:string){
 const counts:Record<AttentionReason,number>={overdue:0,waiting:0,unconfirmed:0,unassigned:0};
 const items:{record:FamilyRecord;reasons:AttentionReason[]}[]=[];
 const health={stale:0,unassigned:0,approaching:0};
 ...
 const severity:AttentionReason[]=['overdue','unconfirmed','waiting','unassigned'];
...
type CompletionAction={type:'create-template';record:FamilyRecord}|{type:'save';record:FamilyRecord & {completeOn?:string}};
export function completionAction(r:FamilyRecord,day:string,records:FamilyRecord[],snapshot?:FamilyRecord[]):CompletionAction{
 if(snapshot){
 ...
 let target=r;
 if(r.recurrence!=='none'&&(!r.id||(records.find(current=>current.id===r.id)||r).status!=='done')){
 ...
 return {type:'save',record:{...target,status:'done',completionDecision:undefined}};
}
...
// Create the template before saving its first completed occurrence.
export async function prepareCompletion(record:FamilyRecord,day:string,records:FamilyRecord[],saveTemplate:(record:FamilyRecord)=>Promise<FamilyRecord|false>,snapshot?:FamilyRecord[]){
 let action=completionAction(record,day,records,snapshot);
 if(action.type==='create-template'){
  ...
  action=completionAction(record,day,records);
 }
...
export function editorSaveAction(r:FamilyRecord,editingDay:string,records:FamilyRecord[],snapshot?:FamilyRecord[]):CompletionAction{
 ...
 if(r.recurrence!=='none'&&(!r.id||(snapshot||records).find(current=>current.id===r.id)?.recurrence==='none')){
  ...
  return {type:'save',record:{...r,status:'done',completeOn:day,completionDecision:undefined}};
 }
 return completionAction(r,day,records,snapshot);
}

const attentionReasonOrder:AttentionReason[]=['overdue','waiting','unconfirmed','unassigned'];
export function attentionReasonCopy(reasons:AttentionReason[]):string{
 const copy:Record<AttentionReason,string>={overdue:'Pasó la fecha',waiting:'Falta una respuesta',unconfirmed:'Falta confirmar',unassigned:'Falta responsable'};
...
export function attentionQuickActions(r:FamilyRecord,reasons:AttentionReason[]):AttentionQuickAction[]{
 ...
 if(canTriageOverdue(r,reasons)){
  actions.push({type:'complete',label:'Hecha'});
