...
export async function saveCompletedTask(raw:any) {
 ...
 const audit=(recordId:string)=>db.prepare('INSERT INTO family_audit(record_id,action,after_data,occurred_at) SELECT id,?,data,? FROM family_records WHERE id=?').bind('create',now,recordId);
 if(raw.id){
