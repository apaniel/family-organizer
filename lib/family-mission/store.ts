...
export async function readRecords() { const db=await database();const result=await db.prepare(`SELECT * FROM family_records WHERE ${plannerRecordsPredicate} ORDER BY updated_at DESC LIMIT 3000`).all();return result.results.map(unpack) as FamilyRecord[]; }
...
export async function saveRecord(raw:any,provenance:CompletionProvenance=defaultProvenance) {
 ...
 if(parent){
  assertPlannerRecord(parent);
  assertPlannerRow(await db.prepare('SELECT * FROM family_records WHERE id=?').bind(parent.id).first());
 }
 ...
 if(!result[0].meta.changes){
  const existing=await db.prepare('SELECT * FROM family_records WHERE source_key=?').bind(data.sourceKey).first();
  assertPlannerRow(existing);
  return unpack(existing);
 }
 return {...data,id,revision:1};
}
export async function removeRecord(id:string,revision:number) {
 assertPlannerRecord({id});
