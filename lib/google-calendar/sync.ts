...
export async function runGoogleCalendarSync() {
    ...
    try {
        ...
        try { response = await fetch(url, { redirect: 'manual', cache: 'no-store', signal: AbortSignal.timeout(30000) }); }
        catch (error) {
            const diagnostic = error instanceof Error ? error.message.replace(String(url), '[private calendar]').replace(/https?:\/\/\S+/g, '[URL]').slice(0, 180) : 'Network failure';
            console.error('Calendar download failed:', diagnostic);
            throw new Error('Could not fetch Google Calendar. Existing events were kept.');
        }
        ...
        if (accountId) await upsertCalendarSyncAccount({ id: accountId, status: 'error', lastErrorAt: new Date().toISOString(), lastErrorMessage: `Google Calendar sync failed (${stage}); existing events were kept.` });
        const message = error instanceof Error ? error.message : '';
        // Restrict diagnostics to our own fixed messages; never include upstream SDK or request details.
        const safeMessage = /^(Could not fetch Google Calendar|Google Calendar returned HTTP [0-9]{3}|Invalid Google Calendar feed|Calendar feed is too large)/.test(message) ? message : `Google Calendar sync failed (${stage}).`;
        console.error('Family calendar sync:', safeMessage);
        throw error;
