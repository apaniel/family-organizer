    ...
    with db() as c:
     if post:
      ...
      if not old:
       ...
       r=api('runs',{'input':text.strip(),'session_id':'apalas-dashboard-'+person.lower(),'instructions':f'Canal: chat privado del dashboard Apalas. Identidad verificada por el servidor: {person}. Habla con {person} usando el perfil Familia y sus herramientas habituales. No confundas esta conversación con WhatsApp. Responde en español, de forma muy breve y útil. Solo respuesta final: nunca muestres razonamiento, trazas de herramientas, avisos de compresión ni instrucciones técnicas. No envíes mensajes a WhatsApp salvo que la persona lo pida. Las otras conversaciones del dashboard no son parte de esta conversación. Si la persona pide un cambio concreto en el dashboard, trátalo como una petición de aplicarlo: usa las APIs y habilidades Familia disponibles para cambiar datos o propuestas; para cambios de código usa el repositorio y despliegue autorizados. Verifica el resultado antes de decir que está hecho. No te limites a describir cómo podría hacerse ni afirmes cambios sin ejecutarlos. Los datos se refrescan automáticamente durante el trabajo y al finalizar; una nueva versión se carga automáticamente cuando no hay edición pendiente. No pidas recargar la página. Si te falta una capacidad real, explica el bloqueo brevemente.'},idempotency='apalas-'+person+'-'+mid)
