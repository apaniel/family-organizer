...
class WorkerTests(unittest.TestCase):
    def test_starts_oldest_queued_message_for_each_parent(self):
        calls, updates = [], []
        messages=[{'id':'q1','person':'Dani','text':'Hazlo','status':'queued','files':[],'created':1}]
    ...
    def test_steers_the_active_run_without_starting_another(self):
        calls, updates = [], []
        messages=[{'id':'p1','person':'Cris','text':'Primero','status':'pending','run':'run_1','files':[],'created':1},{'id':'s1','person':'Cris','text':'Cámbialo','status':'steer_pending','files':[],'created':2}]
    ...
    def test_completes_a_run_and_then_starts_its_queue(self):
        calls, updates = [], []
        messages=[{'id':'p1','person':'Dani','text':'Primero','status':'pending','run':'run_1','files':[],'created':1},{'id':'q1','person':'Dani','text':'Después','status':'queued','files':[],'created':2}]
        def api(path,body=None,key=None):
            calls.append((path,body,key))
